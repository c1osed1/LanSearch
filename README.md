...
### Added
- Добавлен модуль экспорта и импорта тарифов в CSV (`gettarrifs.js`)
- Функция экспорта тарифов с выбором клубов и зон
- Функция импорта тарифов с маппингом клубов/зон и предпросмотром данных
- Поддержка редактирования данных перед применением при импорте

### Fixed
- Исправлен подсчет количества групп дней при экспорте тарифов
- Улучшена проверка количества типов дней при импорте CSV
- Исправлена логика получения данных периодов из модальных окон
- Устранены дубликаты записей при экспорте тарифов

### Changed
- Оптимизирована работа с модальными окнами для получения данных периодов
- Улучшена валидация CSV данных при импорте
- Добавлена проверка соответствия количества типов дней в CSV и системе

## [1.4.7] - 2025-11-30

### Added
- Добавлен модуль массового изменения цен в тарифах (`tariff.js`)
- Режим автоматического поиска по цене для массового обновления
- Режим выборочного изменения с чекбоксами для точного выбора ячеек
- UI блок для управления массовым изменением цен на страницах тарифов

### Fixed
- Исправлена обработка дублирующихся периодов в таблицах тарифов
- Улучшено закрытие модальных окон для корректной обработки всех периодов
- Исправлено всплытие событий при клике на чекбоксы

### Changed
- Улучшена обработка асинхронной загрузки данных тарифов
- Оптимизирована логика получения полных данных периодов через модальные окна
